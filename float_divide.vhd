...
--
-- Lives in float_fpga_hw_test rather than either submodule: it needs
-- hfloat_record from hVHDL_floating_point and lut_reciprocal_pkg from
...
-- mantissas - see testbenches/float_divide_tb.vhd for the measured
-- error.  b = 0 is not handled (matches multiply_add/float_to_fixed:
-- no subnormals/inf/NaN in this format).
--
...
--   ...
--   u_div : entity work.float_divide        -- floatref defaults to hfloat_fp32_zero
...
entity float_divide is
    generic (
        floatref : hfloat_record := hfloat_fp32_zero   -- constrained actual fixes exp/mant widths
    );
    port (
        clock      : in  std_logic;
